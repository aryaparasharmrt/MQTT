...
public class SunStarDS extends Meter {
	...
	private final MeterAddressMap addressMap;

	public SunStarDS(short slaveId, MeterAddressMap addressMap, RTUTCPMasterConnection connection) {
		super(slaveId, connection);
	...
	public boolean connect() {
		return modbusService.writeMultipleRegistersRequest(slaveId, addressMap.getConnectRegisterAddress(),
				new Register[] { new SimpleRegister(addressMap.getConnectRegisterValue()) }, connection);
	}

	@Override
	public boolean disconnect() {
		return modbusService.writeMultipleRegistersRequest(slaveId, addressMap.getDisconnectRegisterAddress(),
	...
	public MeterData readMeter() {
		...
		// Prepare the request
		ReadMultipleRegistersResponse res = modbusService.readMultipleRegistersRequest(slaveId,
		...
		try {
			if (null != res) {
				...
				res = modbusService.readMultipleRegistersRequest(slaveId, addressMap.getOtherRegisterAddress(),
